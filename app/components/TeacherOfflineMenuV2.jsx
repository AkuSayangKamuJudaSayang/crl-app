...
const PLATFORMS = [
  { id: "mac", name: "macOS", icon: ICONS.mac, title: "Install on Mac", steps: ["Open CRL-App in Safari or Chrome.", "Use the browser Install/Add to Dock option when available.", "Keep CRL-App installed so the teacher workspace and saved local data remain available offline."] },
  { id: "windows", name: "Windows", icon: ICONS.windows, title: "Install on Windows", steps: ["Open CRL-App in Edge or Chrome.", "Choose Install CRL-App from the browser address bar or menu.", "Launch CRL-App from the desktop or Start menu even when the internet is unavailable."] },
  { id: "android", name: "Android", icon: ICONS.android, title: "Install on Android", steps: ["Open CRL-App in Chrome.", "Choose Install app or Add to Home screen.", "Open CRL-App from the home screen for the offline teacher workspace."] },
  { id: "ios", name: "iOS", icon: ICONS.ios, title: "Install on iPhone / iPad", steps: ["Open CRL-App in Safari.", "Tap Share, then Add to Home Screen.", "Open the CRL-App icon from the Home Screen to use the installed PWA offline."] },
  { id: "harmony", name: "HarmonyOS", icon: ICONS.harmony, title: "Install on HarmonyOS", steps: ["Open CRL-App in the supported browser on the device.", "Use Add to Home screen / Install app when offered by the browser.", "Open the saved CRL-App from the device launcher while offline."] },
];

function warmOfflineApp() {
  if (typeof navigator === "undefined" || !navigator.serviceWorker) return;
  navigator.serviceWorker.controller?.postMessage({ type: "WARM_CRLA_APP" });
}

export default function TeacherOfflineMenuV2() {
  ...
  useEffect(() => {
    ...
    const findMenu = () => {
      ...
      if (!nav) return false;
      let node = nav.querySelector(".crl-download-nav-mount-v2");
      if (!node) {
        node = document.createElement("div");
        node.className = "crl-download-nav-mount-v2";
  ...
  useEffect(() => {
    ...
    if (/iphone|ipad|ipod/.test(ua)) setSelected("ios");
    else if (/harmonyos|openharmony/.test(ua)) setSelected("harmony");
    else if (/android/.test(ua)) setSelected("android");
    else if (/mac os/.test(ua)) setSelected("mac");
  ...
  return createPortal(
    <>
      ...
      `}</style>
      <button type="button" className="navButton crl-download-nav-button" onClick={() => { warmOfflineApp(); setOpen(true); }} aria-label="Download CRL-App">
            ...
            <div className="crl-platform-grid">
              {PLATFORMS.map((platform) => (
                <button key={platform.id} type="button" className={`crl-platform-button ${selected === platform.id ? "active" : ""}`} onClick={() => setSelected(platform.id)}>
                  <span className="crl-platform-icon-wrap"><img className="crl-platform-icon" src={platform.icon} alt="" aria-hidden="true" /></span>
                  {platform.name}
                </button>
              ))}
            </div>
