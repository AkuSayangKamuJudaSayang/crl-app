...
/*
 * The administrator console uses its own session cookie so that an ordinary
 * teacher/learner session can never open it. See app/api/auth/route.js.
 */
...
export async function requireAdmin(
  ...
  /*
   * This helper reads the teacher/learner cookie, which is never issued with
   * the admin scope. Requiring the scope here makes it fail closed: an
   * ordinary app session belonging to an administrator account cannot be
   * mistaken for an administrator console session. The console uses its own
   * route (app/api/admin) and its own cookie.
   */
  if (user.scope !== "admin") {
    const error =
      new Error(
