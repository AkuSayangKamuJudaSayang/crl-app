...
async function sendRealtimeEvent(code, event, source, payload) {
  ...
  try {
    ...
    /*
     * The channel reported a non-ok send (the websocket may have dropped and
     * the cached channel is no longer joined). Drop the cache so the next
     * publish subscribes a fresh channel instead of silently failing forever,
     * which is how the learner could fall off realtime and only receive
     * updates through the slower polling fallback.
     */
    realtimePublisherChannels.delete(topic);
    try {
      channel.unsubscribe();
