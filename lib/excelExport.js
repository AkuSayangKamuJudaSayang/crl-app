...
/*
 * The template's male/female pie chart reads the FILIPINO summary rows
 * (G8:G9). Since this app only assesses English, repoint it at the English
 * rows (G10:G11) so the pie shows this class's split.
 */
...
/*
 * Every chart keeps a cached copy of its series values. The template ships
 * them all as zeros, so refresh the caches from the values we just wrote.
 * Excel recalculates on open anyway, but this keeps the charts correct in
      ...
      const updated = xml.replace(
        ...
        (whole, inner) => {
          ...
          const rewritten = inner.replace(
            ...
            (point) => {
              const address = addresses[pointIndex];
              pointIndex += 1;
              ...
              return `<c:pt idx="${pointIndex - 1}"><c:v>${formatNumber(
                value
              )}</c:v></c:pt>`;
            }
          );

          return `<c:numRef>${rewritten}</c:numRef>`;
        }
      );
