 ...
 *
 * `$transaction([...])` requires genuine PrismaPromises, and wrapping a write
 * method replaces that with a plain promise, which makes Prisma reject the
 * transaction outright ("all elements of the array need to be Prisma Client
 * promises"). Reads are never passed to a transaction array, so retrying them
 * is safe and still covers the high-frequency paths - the learner and teacher
 * polls - that were the ones surfacing a pooler refusal as an error. Writes keep
 * Prisma's own semantics and their pool_timeout budget.
 */
...
function wrapModelDelegate(delegate) {
  return new Proxy(delegate, {
    get(target, property, receiver) {
      ...
      if (!RETRYABLE_READ_METHODS.has(String(property))) {
