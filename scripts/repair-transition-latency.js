...
const teacherTarget = path.join(process.cwd(), "app", "teacher", "assessment", "AssessmentClient.jsx");
...
if (!teacherSource.includes(teacherMarker)) {
  ...
  if (!patched.includes(markerAnchor)) {
    throw new Error("Existing CRL miscue repair marker was not found; refusing to patch an unexpected teacher assessment client.");
  }
...
const learnerTarget = path.join(process.cwd(), "app", "learner", "LearnerAssessmentPage.jsx");
...
const routeMarker = "CRL_STORY_SELECTION_TIMER_OVERLAY_REPAIR_ROUTE";
let routeSource = fs.readFileSync(routeTarget, "utf8");

if (!routeSource.includes(routeMarker)) {
...
const storyRepairTeacherMarker = "CRL_STORY_SELECTION_TIMER_OVERLAY_REPAIR_TEACHER";
let storyRepairTeacherSource = fs.readFileSync(teacherTarget, "utf8");

if (!storyRepairTeacherSource.includes(storyRepairTeacherMarker)) {
  let patched = storyRepairTeacherSource;
  ...
  patched = replaceOnce(
    ...
    "Immediate selected-story broadcast"
  );
  ...
  patched = replaceOnce(
    ...
    "Final-word save overlay off"
  );

  patched = replaceOnce(
    patched,
    `        {reversionSelecting && reversionSourceWord && (`,
    `        {showWordSavingOverlay && (\n          <div style={{position:"fixed",inset:0,zIndex:7000,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px",background:"rgba(12,32,52,.62)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)"}} role="status" aria-live="assertive" aria-label="Saving assessment">\n            <div style={{width:"min(460px,92vw)",padding:"36px 30px",border:"1px solid #d4e2ed",borderRadius:"24px",background:"linear-gradient(145deg,#f8fbff,#eaf3f9)",boxShadow:"16px 18px 40px rgba(7,25,42,.28),-8px -8px 18px rgba(255,255,255,.78)",textAlign:"center"}}>\n              <div aria-hidden="true" style={{width:"42px",height:"42px",margin:"0 auto 18px",border:"4px solid rgba(21,89,166,.18)",borderTopColor:"#1559a6",borderRadius:"50%",animation:"crlAssessmentSpin .72s linear infinite"}}/>\n              <h2 style={{margin:0,color:"#193c5b",fontSize:"28px",fontWeight:"950"}}>Saving, please wait...</h2>\n              <p style={{margin:"10px 0 0",color:"#6d8498",fontSize:"14px",lineHeight:1.6}}>Saving the final Word Recognition result before opening the story passage choices.</p>\n            </div>\n          </div>\n        )}\n\n        {reversionSelecting && reversionSourceWord && (`,
    "Final-word full-screen saving overlay"
  );

  patched = patched.replace(
    "      <main className=\"teacherAssessmentPage\"",
    `      /* ${storyRepairTeacherMarker} */\n      <main className="teacherAssessmentPage"`
  );
