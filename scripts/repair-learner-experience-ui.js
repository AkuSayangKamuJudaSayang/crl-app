...
if (!learner.includes(learnerMarker)) {
  ...
  if (!ratingGridPattern.test(block)) {
    throw new Error("Learner experience display repair: expected rating grid was not found.");
  }

  const ratingGridReplacement = `<div className="rating-grid" aria-label="Learner experience ratings">\n              {[\n                ["😟", 1], ["🙁", 2], ["😐", 3], ["🙂", 4], ["🤩", 5],\n              ].map(([emoji, rating]) => (\n                <div key={rating} className={\`rating-button rating-display\${selectedExperienceRating === rating ? " selected" : ""}\`} role="img" aria-label={\`Rating \${rating} out of 5\`} aria-disabled="true">\n                  <span className="rating-emoji">{emoji}</span>\n                  <span className="rating-number">{rating}</span>\n                </div>\n              ))}\n            </div>\n\n            {savingExperienceRating &&`;
  ...
  if (!learner.includes(".rating-display {")) {
    learner = learner.replace(styleAnchor, `        .rating-display {\n          cursor: default !important;\n          pointer-events: none !important;\n          user-select: none;\n        }\n\n${styleAnchor}`);
  }
  learner = learner.replace(/\n      \{showConnectionSettings/, `\n      {/* ${learnerMarker} */}\n\n      {showConnectionSettings`);
  write(learnerPath, learner);
}

console.log("Applied CRL learner experience UI repair: teacher controls the five emoji ratings; learner sees the same five choices as non-interactive display only.");
